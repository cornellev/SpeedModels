...
def train_one_epoch(model, dataloader, optimizer, device):
    ...
    for images, labels in dataloader:
        ...
        labels = labels.long()
        #weight = torch.tensor([100, 1], dtype=torch.float).to(device)
...
def train_model(epochs, model, training_loader, optimizer, device, resume_path=None):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if loss < best_loss:
            best_loss = loss
            save_model(model, optimizer, epoch, loss, 'models/doublescuffed/best_model.pth')
...
train_model(100, model, training_loader, optimizer, device, resume_path)
